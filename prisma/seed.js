...
async function main() {
    ...
    for (let i = 0; i < 50; i++) {
        await prisma.visitor.create({
            data: {
                siteId: site.id,
                country: (Math.random() > 0.1 ? ['US', 'UK', 'CA', 'IN', 'AU'][Math.floor(Math.random() * 5)] : null),
                source: (Math.random() > 0.1 ? ['Google', 'Direct', 'Twitter', 'LinkedIn'][Math.floor(Math.random() * 4)] : null),
            },
        });
    }
