...
import WhatsAppToken from "../../../../models/Campaign/WhatsappCampaign/Meta/metaWhatsappCampaignTokenSchema.js";
...
} from "../../../../services/MetaWhatsapp/metaOAuth.services.js";
...
export const fetchConnectedNumber = async (req, res, next) => {
    try {
        ...
        const ConnectedUsersPhone = await WhatsAppToken.find({ userId: req.user.id });
...
export const connectEmbeddedWhatsapp = async (req, res) => {
    ...
    try {
        ...
        await axios.post(
            ...
            {},
            { params: { access_token: config.META_WHATSAPP_SYSTEM_USER_TOKEN } }
        ).catch(e => console.warn('Subscribe warning:', e.response?.data))
        ...
        for (const phone of phoneData.data || []) {
            try {
                ...
                console.log(`✅ Saved ${phone.display_phone_number} for user ${userId}`)
            } catch (saveErr) {
