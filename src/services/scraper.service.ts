...
const getTurndownScript = async () => {
  ...
  try {
    const res = await axios.get("https://unpkg.com/turndown/dist/turndown.js");
    cachedTurndownScript = res.data;
    return cachedTurndownScript;
  } catch (err) {
    console.error(
      "[SCRAPER] Failed to fetch Turndown from CDN. Fallbacks will be used.",
    );
    return "";
  }
};
...
export const scrapeWebsite = async (formattedUrl: string): Promise<any> => {
  ...
  try {
      ...
      try {
        ...
        try {
          ...
          if (
            ...
            console.log(
              `[SCRAPER] Network error for ${urlToVisit}, skipping...`,
            );
            return { error: "NETWORK_ERROR", message: gotoErr.message }; // Return structured error
          }
          console.log(
            `[SCRAPER] Navigation timeout for ${urlToVisit}, proceeding to extract current DOM...`,
          );
        }
        ...
        try {
          ...
          console.log(
            `[SCRAPER] Timeout waiting for text on ${urlToVisit}. Proceeding with current DOM.`,
          );
        }
        ...
        const evaluatedData = await page.evaluate(
          (currentUrl, hasTurndown) => {
            const errorCodeDiv = document.querySelector(".error-code");
            const isErrorPage =
              document.title.includes("site can’t be reached") ||
        ...
        if (evaluatedData && evaluatedData.error === "CHROME_ERROR") {
          console.error(
            `[SCRAPER] Chrome Error Page detected for ${urlToVisit}: ${evaluatedData.errorCode}`,
          );
    ...
    console.log(
      `[SCRAPER] Custom Scraping Finished. Total markdown length: ${firecrawlLikeData.markdown.length}`,
    );
    await browser.close();
    ...
    if (
      ...
      throw new Error(
        `Failed to extract any content from ${formattedUrl}. The site might be blocking headless browsers or the URL might be invalid.`,
      );
    }
