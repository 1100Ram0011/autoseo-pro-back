...
const parseLeadJson = (raw: string) => {
...
const shapeLead = (lead: any, targetMarket: string, geographicFocus: string, index: number) => {
...
const fetchOneBatch = async (targetMarket: string, geographicFocus: string, batchSize: number, batchNum: number) => {
...
export const fetchLeadsWithGemini = async (targetMarket: string, geographicFocus: string, limit: number = 10) => {
  ...
  for (let lead of finalLeads) {
    if (lead.linkedin !== "N/A") {
      try {
        await axios.head(lead.linkedin, {
          timeout: 4000,
...
export const suggestTargetMarketsFromGemini = async (businessName: string, industry: string, businessDescription: string, input: string) => {
...
export const validateTargetMarketWithGemini = async (businessName: string, industry: string, businessDescription: string, targetMarket: string) => {
