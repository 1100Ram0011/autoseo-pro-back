...
exports.executeAiAction = exports.analyzeOmnichannelState = void 0;
const genai_1 = require("@google/genai");
const prisma_1 = __importDefault(require("../config/prisma"));
const ai = new genai_1.GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY || 'MOCK_KEY' });
const analyzeOmnichannelState = async (siteId, masterState) => {
    try {
        let anomalies = [];
      ...
      `;
        ...
        for (const anomaly of anomalies) {
            const dbAnomaly = await prisma_1.default.aiAnomaly.create({
                data: {
                    siteId,
                    type: anomaly.type,
            ...
            if (anomaly.actions && Array.isArray(anomaly.actions)) {
                for (const action of anomaly.actions) {
                    await prisma_1.default.aiAction.create({
                        data: {
...
exports.analyzeOmnichannelState = analyzeOmnichannelState;
const executeAiAction = async (actionId) => {
    const action = await prisma_1.default.aiAction.findUnique({ where: { id: actionId }, include: { anomaly: true } });
    if (!action)
        return;
    // Simulate complex cross-platform fixes
    await new Promise(resolve => setTimeout(resolve, 2000));
    await prisma_1.default.aiAction.update({
        where: { id: actionId },
        data: { isExecuted: true }
    });
    const remainingActions = await prisma_1.default.aiAction.count({
        where: { anomalyId: action.anomalyId, isExecuted: false }
    });
    if (remainingActions === 0) {
        await prisma_1.default.aiAnomaly.update({
