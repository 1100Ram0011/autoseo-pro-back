...
exports.runFullAnalysis = exports.analyzeUrl = void 0;
// @ts-nocheck
const googleapis_1 = require("googleapis");
const getPageSpeedClient = () => {
    return googleapis_1.google.pagespeedonline('v5');
};
...
const extractFieldMetric = (metric) => {
    if (!metric)
        return null;
    return {
...
const extractWebVitals = (audits, loadingExperience, originLoadingExperience) => {
    const lab = {
...
const extractSpecificAudits = (audits, keys) => {
    return keys.map(key => {
        const audit = audits[key];
        if (!audit)
            return null;
...
const extractCategories = (audits, categories) => {
...
const analyzeUrl = async (url, strategy) => {
    try {
...
const runFullAnalysis = async (url) => {
    console.log(`Starting Full PSI Analysis for ${url}...`);
    const [mobileData, desktopData] = await Promise.all([
        (0, exports.analyzeUrl)(url, 'mobile'),
        (0, exports.analyzeUrl)(url, 'desktop')
    ]);
    return {
        url,
