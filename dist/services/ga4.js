...
exports.getGa4Overview = exports.getAnalyticsData = exports.getGa4AccountsAndProperties = void 0;
exports.formatAnalytics = formatAnalytics;
// @ts-nocheck
const googleapis_1 = require("googleapis");
const prisma_1 = __importDefault(require("../config/prisma"));
const getGa4AccountsAndProperties = async (auth) => {
    const admin = googleapis_1.google.analyticsadmin({ version: 'v1beta', auth });
    try {
...
exports.getGa4AccountsAndProperties = getGa4AccountsAndProperties;
const DATE_RANGES = {
...
const getAnalyticsData = async (propertyId, client, range = "weekly", customStart = null, customEnd = null) => {
    ...
    const [report, devices, countries, sources, pages, realtime, trend, newReturning, browsers, operatingSystems, landingPages, exitPages, conversions,] = await Promise.all([
...
function formatAnalytics(data) {
    ...
    (data?.geoHierarchy?.rows || []).forEach((row) => {
        ...
        if (skip(country))
            return;
        if (!geoTree[country])
            geoTree[country] = { users: 0, regions: {} };
    ...
    rtRows.forEach((row) => {
        ...
        if (city !== "(not set)")
            rtByCity[city] = (rtByCity[city] || 0) + users;
...
const getGa4Overview = async (propertyId, userId) => {
    ...
    try {
        const data = await (0, exports.getAnalyticsData)(propertyId, oauth2Client, 'monthly');
        return {
            metrics: {
