...
exports.analyzeInsights = exports.getGscSites = exports.inspectUrl = exports.getGscSitemaps = exports.getGscQuery = exports.getGscCoverage = exports.getGscDevices = exports.getGscCountries = exports.getGscPages = exports.getGscKeywords = exports.getGscOverview = exports.resolveGscSiteUrl = exports.getAuth = void 0;
// @ts-nocheck
 ...
 */
const resolveGscSiteUrl = async (dbSiteUrl, userId) => {
    ...
    if (gscSiteUrlCache.size > 100) {
        ...
        for (const [key, entry] of gscSiteUrlCache) {
            if (entry.expiresAt <= now)
                gscSiteUrlCache.delete(key);
        }
    }
    const auth = await (0, exports.getAuth)(userId);
    if (!auth)
        return dbSiteUrl;
    try {
...
exports.resolveGscSiteUrl = resolveGscSiteUrl;
...
const getGscOverview = async (siteUrl, userId) => {
    const auth = await (0, exports.getAuth)(userId);
    if (!auth) {
        console.log('No GSC credentials found, returning mock overview data.');
        return MOCK_DATA.overview;
    }
    siteUrl = await (0, exports.resolveGscSiteUrl)(siteUrl, userId);
    const searchconsole = googleapis_1.google.searchconsole({ version: 'v1', auth });
...
exports.getGscOverview = getGscOverview;
const getGscKeywords = async (siteUrl, userId, siteId) => {
    const auth = await (0, exports.getAuth)(userId);
    if (!auth) {
        if (siteId) {
            const dbKeywords = await prisma_1.default.keyword.findMany({ where: { siteId } });
...
exports.getGscKeywords = getGscKeywords;
const getGscPages = async (siteUrl, userId, siteId) => {
    const auth = await (0, exports.getAuth)(userId);
    if (!auth) {
...
exports.getGscPages = getGscPages;
const getGscCountries = async (siteUrl, userId) => {
    const auth = await (0, exports.getAuth)(userId);
    if (!auth)
        return { countries: [] };
    siteUrl = await (0, exports.resolveGscSiteUrl)(siteUrl, userId);
...
exports.getGscCountries = getGscCountries;
const getGscDevices = async (siteUrl, userId) => {
    const mockDevices = {
        devices: [
...
exports.getGscDevices = getGscDevices;
const getGscCoverage = async (siteUrl, userId) => {
    // Note: GSC Indexing API does not easily return aggregate counts of "indexed" vs "not indexed"
    return MOCK_DATA.coverage;
};
exports.getGscCoverage = getGscCoverage;
// =========================================================================
// ADVANCED GSC API INTEGRATIONS
// =========================================================================
const getGscQuery = async (siteUrl, userId, options) => {
    const auth = await (0, exports.getAuth)(userId);
    if (!auth)
        return { rows: [] };
    siteUrl = await (0, exports.resolveGscSiteUrl)(siteUrl, userId);
...
exports.getGscQuery = getGscQuery;
const getGscSitemaps = async (siteUrl, userId) => {
    const auth = await (0, exports.getAuth)(userId);
    if (!auth)
        return { sitemaps: [] };
    siteUrl = await (0, exports.resolveGscSiteUrl)(siteUrl, userId);
...
exports.getGscSitemaps = getGscSitemaps;
const inspectUrl = async (siteUrl, url, userId) => {
    const auth = await (0, exports.getAuth)(userId);
    if (!auth)
...
const getGscSites = async (userId) => {
    const auth = await (0, exports.getAuth)(userId);
    if (!auth)
        return { siteEntry: [] };
    const searchconsole = googleapis_1.google.searchconsole({ version: 'v1', auth });
...
exports.getGscSites = getGscSites;
const analyzeInsights = async (siteUrl, userId) => {
    const auth = await (0, exports.getAuth)(userId);
    if (!auth)
        return { insights: [] };
    siteUrl = await (0, exports.resolveGscSiteUrl)(siteUrl, userId);
...
exports.analyzeInsights = analyzeInsights;
